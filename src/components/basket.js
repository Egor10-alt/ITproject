    ...
    renderBasket = config => {
      ...
      elem.addEventListener('click', event => {
        ...
        if (event.target.classList.contains('basket-card-down')) {
          ...
          // this.#totalPay.textContent = Number(count.textContent) !== 0 ? `${Number(this.#totalPay.textContent.substring(0, this.#totalPay.textContent.indexOf(' '))) - config.price} UA.` ;
        }
      });
      // const elemName = document.querySelectorAll('.basket-card-name');
